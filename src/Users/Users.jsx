...
const Users = (props) => {
    ...
    let userElements = props.users.map(u => <li key={u.id} className={s.user}>
            ...
            {u.followed
                ? <button onClick={() => {props.unfollowUser(u.id)}} className={s.followButton}>Unfollow</button> :
                <button onClick={() => {props.followUser(u.id)}} className={s.followButton}>Follow</button>}
    </li>);
    return (
